...
<section id="work-detail" class="section work-detail">
  <?php 
    ...
    $result = $conn->query("SELECT * FROM tblproduct WHERE id='$post_id'");
      while($row = $result->fetch_assoc()) { ?>
      <div class="container-fluid">
        <div class="row">
          <div class="col-md-8 work-detail-margin detail-image">
            <div class="work-image">
              <?php if (isset($row["image"])) { ?><p> <img src="upload/<?php echo $row["image"]; ?>" style="width:100%;" alt="<?php echo $row["name"]; ?>"> </p><?php } ?>
            </div>
          </div>
          <div class="col-md-4 work-detail-margin detail-contentbox">
            <div class="detail-content">
              <?php if (isset($row["name"])) { ?><h4><?php echo $row["name"]; ?></h4><?php } ?>
              <?php if (isset($row["price"])) { ?><?php echo "<strong>Price: </strong>". $row["price"]."<br/>"; ?><?php } ?>
